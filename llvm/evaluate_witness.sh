#!/bin/bash
TESTS=$(find test/Transforms/SimplifyCFG -name "*.ll")
TOTAL=0
PASS=0
FAIL=0
CRASH=0
...
for test in $TESTS; do
    TOTAL=$((TOTAL+1))
    OUTPUT=$(../build/bin/opt -load-pass-plugin=../build/lib/WitnessPasses.so -passes="simplifycfg-witness" "$test" -disable-output 2>&1)
    if [ $? -ne 0 ]; then
        CRASH=$((CRASH+1))
