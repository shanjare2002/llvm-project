...
static bool performBlockTailMerging(
  ...
  for (BasicBlock *BB : BBs) {
    ...
    // Record the merge: source block (BB) maps to canonical block (CanonicalBB)
    if (blockMap)
...
static bool tailMergeBlocksWithSimilarFunctionTerminators(
  ...
  for (BasicBlock &BB : F) {
    ...
    // We can't taipl-merge block that contains a musttail call.
...
simplifyFunctionCFG(Function &F, const TargetTransformInfo &TTI,
  ...
  bool Changed = simplifyFunctionCFGImpl(F, TTI, DT, Options, blockMap);
...
LLVM_ABI PreservedAnalyses SimplifyCFGPass::run(Function &F,
  ...
  // Collect all blocks before the pass
  SmallVector<BasicBlock *, 16> AllBlocksBefore;
  for (BasicBlock &BB : F)
    AllBlocksBefore.push_back(&BB);

  // Internal blockMap for tracking (not exposed)
  DenseMap<BasicBlock *, BasicBlock *> blockMap;

  if (!simplifyFunctionCFG(F, TTI, DT, Options, &blockMap)) {
    errs() << "\n=== SimplifyCFG: No changes made ===\n";
    // If no changes were made, record identity mappings for untouched blocks
    for (BasicBlock *BB : AllBlocksBefore)
      blockMap[BB] = BB;

    // Log the block mapping even when no changes
