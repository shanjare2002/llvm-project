...
namespace {
static const bool RequireAndPreserveDomTree = false;
static const unsigned PHINodeFoldingThreshold = 2;
static const unsigned TwoEntryPHINodeFoldingThreshold = 4;
static const bool HoistCommon = true;
static const bool HoistLoadsWithCondFaulting = true;
static const bool HoistStoresWithCondFaulting = true;
static const unsigned HoistLoadsStoresWithCondFaultingThreshold = 6;
static const unsigned HoistCommonSkipLimit = 20;
static const bool SinkCommon = true;
static const bool HoistCondStores = true;
static const bool MergeCondStores = true;
static const bool MergeCondStoresAggressively = false;
static const bool SpeculateOneExpensiveInst = true;
static const unsigned MaxSpeculationDepth = 10;
static const int MaxSmallBlockSize = 10;
static const unsigned BranchFoldThreshold = 2;
static const unsigned BranchFoldToCommonDestVectorMultiplier = 2;
static const bool EnableMergeCompatibleInvokes = true;
static const unsigned MaxSwitchCasesPerResult = 16;
} // namespace
...
static void mergeCompatibleInvokesImpl(ArrayRef<InvokeInst *> Invokes,
  ...
  for (Use &U : MergedInvoke->operands()) {
    ...
    PHINode *PN =
        PHINode::Create(U->getType(), /*NumReservedValues=*/Invokes.size(), "",
...
bool SimplifyCFGOpt::simplifySingleResume(ResumeInst *RI) {
  ...
  // Turn all invokes that unwind here into calls and delete the basic
  // block.
...
static bool removeEmptyCleanup(CleanupReturnInst *RI, DomTreeUpdater *DTU) {
  // If this is a trivial cleanup pad that executes no instructions, it can
  // be eliminated.  If the cleanup pad continues to the caller, any
  // predecessor that is an EH pad will be updated to continue to the caller
  // and any predecessor that terminates with an invoke instruction will
  // have its invoke instruction converted to a call instruction.  If the
  // cleanup pad being simplified does not continue to the caller, each
  // predecessor will be updated to continue to the unwind destination of
  // the cleanup pad being simplified.
  BasicBlock *BB = RI->getParent();
  ...
  // Check that there are no other instructions except for benign
  // intrinsics.
  if (!isCleanupBlockEmpty(
  ...
  // control flow avoids some potentially slow checks, since we can
  // currently be certain that UnwindDest and BB have no common predecessors
  // (since they are both EH pads).
  if (UnwindDest) {
    // First, go through the PHI nodes in UnwindDest and update any nodes
    // that reference the block we are removing
  ...
  // We use make_early_inc_range here because we will remove all
  // predecessors.
...
static bool mergeCleanupPad(CleanupReturnInst *RI) {
  ...
  // This cleanupret isn't the only predecessor of this cleanuppad, it
  // wouldn't be safe to merge without code duplication.
...
bool SimplifyCFGOpt::simplifyCleanupReturn(CleanupReturnInst *RI) {
  // It is possible to transiantly have an undef cleanuppad operand because
  // we have deleted some, but not all, dead blocks. Eventually, this block
  // will be deleted.
...
bool SimplifyCFGOpt::simplifyUnreachable(UnreachableInst *UI) {
  ...
  // Ensure that any debug-info records that used to occur after the
  // Unreachable are moved to in front of it -- otherwise they'll "dangle"
  ...
  // If there are any instructions immediately before the unreachable that
  // can be removed, do so.
  while (UI->getIterator() != BB->begin()) {
    ...
    // Note that deleting EH's here is in fact okay, although it involves a
    // bit of subtle reasoning. If this inst is an EH, all the predecessors
    // of this block will be the unwind edges of
    // Invoke/CatchSwitch/CleanupReturn, and we can therefore guarantee this
    ...
    if (auto *BI = dyn_cast<BranchInst>(TI)) {
      ...
      if (CSI->getNumHandlers() == 0) {
        if (CSI->hasUnwindDest()) {
          // Redirect all predecessors of the block containing
          // CatchSwitchInst to instead branch to the CatchSwitchInst's
          // unwind destination.
          if (DTU) {
...
bool SimplifyCFGOpt::turnSwitchRangeIntoICmp(SwitchInst *SI,
  ...
  if (!DestB)
    return false; // All destinations are the same and the default is
                  // unreachable
  ...
  Value *Cmp;
  // If NumCases overflowed, then all possible values jump to the
  // successor.
  ...
  // Clean up the default block - it may have phis or other instructions
  // before the unreachable terminator.
  if (!HasDefault)
...
static bool eliminateDeadSwitchCases(SwitchInst *SI, DomTreeUpdater *DTU,
  ...
  // We can also eliminate cases by determining that their values are
  // outside of the limited range of the condition based on how many
  // significant (non-sign) bits are in the condition value.
  ...
  // some of the bits in the value, we can use that to more precisely
  // compute the number of possible unique case values.
  bool HasDefault = !SI->defaultDestUnreachable();
  ...
  if (HasDefault && DeadCases.empty() &&
    ...
    // When only one case value is missing, replace default with that
    // case. Eliminating the default branch will provide more
    // opportunities for optimization, such as lookup tables.
...
/// TryToSimplifyUncondBranchFromEmptyBlock (i.e. it is empty and
/// terminated by an unconditional branch), look at the phi node for BB in
/// the successor block and see if the incoming value is equal to
/// CaseValue. If so, return the phi node, and set PhiIndex to BB's index
/// in the phi node.
static PHINode *findPHIForConditionForwarding(ConstantInt *CaseValue,
                                              BasicBlock *BB, int *PhiIndex) {
  if (&*BB->getFirstNonPHIIt() != BB->getTerminator())
    return nullptr; // BB must be empty to be a candidate for
                    // simplification.
  if (!BB->getSinglePredecessor())
...
/// dominated by the switch, if that would mean that some of the
/// destination blocks of the switch can be folded away. Return true if a
/// change is made.
static bool forwardSwitchConditionToPHI(SwitchInst *SI) {
  ...
  for (const auto &Case : SI->cases()) {
    ...
    // Replace phi operands in successor blocks that are using the
    // constant case value rather than the switch condition variable:
    //   switchbb:
    ...
    for (PHINode &Phi : CaseDest->phis()) {
      // This only works if there is exactly 1 incoming edge from the
      // switch to a phi. If there is >1, that means multiple cases of the
      // switch map to 1 value in the phi, and that phi value is not the
      // switch condition. Thus, this transform would not make sense (the
      // phi would be invalid because a phi can't have different incoming
      // values from the same block).
    ...
    // Collect phi nodes that are indirectly using this switch's case
    // constants.
    int PhiIdx;
  ...
  for (auto &ForwardingNode : ForwardingNodes) {
    ...
    if (Indexes.size() < 2 &&
        !llvm::is_contained(Phi->incoming_values(), SI->getCondition()))
      continue;
...
/// Returns the resulting constant on success, 0 otherwise.
static Constant *
...
/// at the common destination basic block, *CommonDest, for one of the
/// case destionations CaseDest corresponding to value CaseVal (0 for the
...
getCaseResults(SwitchInst *SI, ConstantInt *CaseVal, BasicBlock *CaseDest,
  ...
  // If CaseDest is empty except for some side-effect free instructions
  // through which we can constant-propagate the CaseVal, continue to its
  // successor.
  ...
  for (Instruction &I : CaseDest->instructionsWithoutDebug(false)) {
    if (I.isTerminator()) {
      ...
      // If the instruction has uses outside this block or a phi node slot
      // for the block, it is not safe to bypass the instruction since it
      // would then no longer dominate all its uses.
      for (auto &Use : I.uses()) {
...
static bool initializeUniqueCases(SwitchInst *SI, PHINode *&PHI,
  ...
  getCaseResults(SI, nullptr, SI->getDefaultDest(), &CommonDest, DefaultResults,
                 DL, TTI);
  // If the default value is not found abort unless the default
...
// Helper function that checks if it is possible to transform a switch
// with only two cases (or two cases + default) that produces a result
// into a select.
// TODO: Handle switches with more than 2 cases that map to the same
// result.
static Value *foldSwitchToSelect(const SwitchCaseResultVectorTy &ResultVector,
  ...
  // Handle the degenerate case where two cases have the same result
  // value.
  if (ResultVector.size() == 1 && DefaultResult) {
    ...
    if (isPowerOf2_32(CaseCount)) {
      ...
      // Find the minimum value and compute the and of all the case
      // values.
...
// Helper function to cleanup a switch instruction that has been converted
// into a select, fixing up PHI nodes and basic blocks.
...
/// If a switch is only used to initialize one or more phi nodes in a
/// common successor block with only two different constant values, try to
/// replace the switch with a select. Returns true if the fold was made.
static bool trySwitchToSelect(SwitchInst *SI, IRBuilder<> &Builder,
  ...
  Constant *DefaultResult;
  SwitchCaseResultVectorTy UniqueResults;
  // Collect all the cases that will deliver the same value from the
  // switch.
...
/// This class represents a lookup table that can be used to replace a
/// switch.
class SwitchLookupTable {
public:
  /// Create a lookup table to use as a switch replacement with the
  /// contents of Values, using DefaultValue to fill any holes in the
  /// table.
  SwitchLookupTable(
...
private:
  ...
  // different ways.
  enum {
    // For tables where each element contains the same value, we just have
    // to
    ...
    // For small tables with integer elements, we can pack them into a
    // bitmap
...
SwitchLookupTable::SwitchLookupTable(
  ...
  // If each element in the table contains the same value, we only need to
  // store that single value.
  if (SingleValue) {
    Kind = SingleValueKind;
    return;
  }

  // Check if we can derive the value with a linear transformation from
  // the table index.
  if (isa<IntegerType>(ValueType)) {
    ...
    for (uint64_t I = 0; I < TableSize; ++I) {
      ...
      if (!ConstVal && isa<PoisonValue>(TableContents[I])) {
        ...
        // To prevent any regressions from before we switched to using
        // poison as the default value, holes will fall back to using the
        // first value. This can be removed once we add proper handling
        // for poisons in lookup tables.
  ...
  // Set the alignment to that of an array items. We will be only loading
  // one value out of it.
...
Value *SwitchLookupTable::buildLookup(Value *Index, IRBuilder<> &Builder,
  ...
  case BitMapKind: {
    ...
    // Multiply the shift amount by the element width. NUW/NSW can always
    // be set, because wouldFitInRegister guarantees Index * ShiftAmt is
    // in BitMap's bit width.
...
bool SwitchLookupTable::wouldFitInRegister(const DataLayout &DL,
  ...
  // values are <= 15, we could try to narrow the type.
...
static bool isTypeLegalForLookupTable(Type *Ty, const TargetTransformInfo &TTI,
  ...
  // Also allow power of 2 integer types that have at least 8 bits and fit
  // in a register. These types are common in frontend languages and
  // targets usually support loads of these types.
  // TODO: We could relax this to any integer that fits in a register and
  // rely on ABI alignment and padding in the table to allow the load to
  // be widened. Or we could widen the constants and truncate the load.
...
static bool isSwitchDense(uint64_t NumCases, uint64_t CaseRange) {
  // 40% is the default density for building a jump table in
  // optsize/minsize mode. See also
  // TargetLoweringBase::isSuitableForJumpTable(), which this function was
  // based on.
...
/// Determine whether a lookup table should be built for this switch,
/// based on the number of cases, size of the table, and the types of the
/// results.
// TODO: We could support larger than legal types by limiting based on the
// number of loads required and/or table size. If the constants are small
// we could use smaller table entries and extend after the load.
static bool
shouldBuildLookupTable(SwitchInst *SI, uint64_t TableSize,
  ...
  // Don't build a table that doesn't fit in-register if it has illegal
  // types.
  if (HasIllegalType)
    return false;
...
static void reuseTableCompare(
  ...
  // We require that the compare is in the same block as the phi so that
  // jump threading can do its work afterwards.
  ...
  // Check if the compare with the default value is constant true or
  // false.
  ...
  // Check if the compare with the case values is distinct from the
  ...
  // Although this check is invariant in the calling loops, it's better to
  // do it at this late stage. Practically we do it at most once for a
  // switch.
  ...
  if (DefaultConst == FalseConst) {
    ...
    // The compare yields the same result, just inverted. We can replace
    // it.
...
/// If the switch is only used to initialize one or more phi nodes in a
/// common successor block with different constant values, replace the
/// switch with lookup tables.
static bool switchToLookupTable(SwitchInst *SI, IRBuilder<> &Builder,
  ...
  // could split off a dense part and build a lookup table for that.

  // FIXME: This creates arrays of GEPs to constant strings, which means
  // each GEP needs a runtime relocation in PIC code. We should just build
  // one big string and lookup indices into that.

  // Ignore switches with less than three cases. Lookup tables will not
  // make them faster, so we don't analyze them.
  if (SI->getNumCases() < 3)
    return false;

  // Figure out the corresponding result for each case value and phi node
  // in the common destination, as well as the min and max case values.
  ...
  // case or a bitmask that fits in a register.
  ...
  // If the default destination is unreachable, or if the lookup table
  // covers all values of the conditional variable, branch directly to the
  // lookup table BB. Otherwise, check that the condition is within the
  // case range.
  bool DefaultIsReachable = !SI->defaultDestUnreachable();

  bool TableHasHoles = (NumResults < TableSize);

  // If the table has holes but the default destination doesn't produce
  // any constant results, the lookup table entries corresponding to the
  // holes will contain poison.
  bool AllHolesArePoison = TableHasHoles && !HasDefaultResults;

  // If the default destination doesn't produce a constant result but is
  // still reachable, and the lookup table has holes, we need to use a
  // mask to determine if the current index should load from the lookup
  // table or jump to the default case. The mask is unnecessary if the
  // table has holes but the default destination is unreachable, as in
  // that case the holes must also be unreachable.
  ...
  // Compute the maximum table size representable by the integer type we
  // are switching upon.
  ...
  if (UseSwitchConditionAsTableIndex) {
    ...
    // If the default is unreachable, all case values are s>= MinCaseVal.
    // Then we can try to attach nsw.
    bool MayWrap = true;
  ...
  // check. It will use the default result to fill in the table hole
  // later, so make sure it exist.
  if (UseSwitchConditionAsTableIndex && HasDefaultResults) {
    ...
    // wouldFitInRegister.
    // TODO: Consider growing the table also when it doesn't fit in a
    // register if no optsize is specified.
    ...
    if (!CR.isUpperWrapped() && all_of(ResultTypes, [&](const auto &KV) {
      ...
      // There may be some case index larger than the UpperBound
      // (unreachable case), so make sure the table size does not get
      // smaller.
      TableSize = std::max(UpperBound, TableSize);
      // The default branch is unreachable after we enlarge the lookup
      // table. Adjust DefaultIsReachable to reuse code path.
  ...
  if (!DefaultIsReachable || GeneratingCoveredLookupTable) {
    ...
    // Note: We call removeProdecessor later since we need to be able to
    // get the PHI value for the default case in case we're using a bit
    // mask.
  } else {
  ...
  if (NeedMask) {
    // Before doing the lookup, we do the hole check. The LookupBB is
    // therefore re-purposed to do the hole check, and we create a new
    // LookupBB.
  ...
  if (!DefaultIsReachable || GeneratingCoveredLookupTable) {
    // We cached PHINodes in PHIs. To avoid accessing deleted PHINodes
  ...
  for (PHINode *PHI : PHIs) {
    ...
    // Do a small peephole optimization: re-use the switch table compare
    // if possible.
...
/// Try to transform a switch that has "holes" in it to a contiguous
/// sequence of cases.
///
/// A switch such as: switch(i) {case 5: case 9: case 13: case 17:} can be
/// range-reduced to: switch ((i-5) / 4) {case 0: case 1: case 2: case
/// 3:}.
///
...
static bool reduceSwitchRange(SwitchInst *SI, IRBuilder<> &Builder,
  ...
  if (CondTy->getIntegerBitWidth() > 64 ||
      !DL.fitsInLegalInteger(CondTy->getIntegerBitWidth()))
    return false;
  // Only bother with this optimization if there are more than 3 switch
  // cases; SDAG will only bother creating jump tables for 4 or more
  // cases.
  if (SI->getNumCases() < 4)
    return false;

  // This transform is agnostic to the signedness of the input or case
  // values. We can treat the case values as signed or unsigned. We can
  // optimize more common cases such as a sequence crossing zero
  // {-4,0,4,8} if we interpret case values as signed.
  ...
  // Now we have signed numbers that have been shifted so that, given
  // enough precision, there are no negative values. Since the rest of the
  // transform is bitwise only, we switch now to an unsigned
  // representation.

  // This transform can be done speculatively because it is so cheap - it
  // results in a single rotate operation being inserted.

  // countTrailingZeros(0) returns 64. As Values is guaranteed to have
  // more than one element and LLVM disallows duplicate cases, Shift is
  // guaranteed to be less than 64.
  unsigned Shift = 64;
  for (auto &V : Values)
  ...
  // The obvious transform is to shift the switch condition right and emit
  // a check that the condition actually cleanly divided by GCD, i.e.
  //   C & (1 << Shift - 1) == 0
  // inserting a new CFG edge to handle the case where it didn't divide
  // cleanly.
  //
  // A cheaper way of doing this is a simple ROTR(C, Shift). This performs
  // the shift and puts the shifted-off bits in the uppermost bits. If any
  // of these are nonzero then the switch condition will be very large and
  // will hit the default case.
...
///
/// This transformation allows better lowering and may transform the
/// switch instruction into a sequence of bit manipulation and a smaller
/// log2(C)-indexed value table (instead of traditionally emitting a load
/// of the address of the jump target, and indirectly jump to it).
static bool simplifySwitchOfPowersOfTwo(SwitchInst *SI, IRBuilder<> &Builder,
  ...
  // Only bother with this optimization if there are more than 3 switch
  // cases. SDAG will start emitting jump tables for 4 or more cases.
  if (SI->getNumCases() < 4)
    return false;

  // We perform this optimization only for switches with
  // unreachable default case.
  // This assumtion will save us from checking if `Condition` is a power
  // of two.
...
/// Fold switch over ucmp/scmp intrinsic to br if two of the switch arms
...
/// Checking whether two cases of SI are equal depends on the contents of
...
/// especially since the same BasicBlock may be passed as an argument
/// multiple times. To do this, we can precompute a map of PHINode -> Pred
/// BasicBlock -> IncomingValue and add it in the Wrapper so isEqual can
/// do O(1) checking of the incoming values.
...
template <> struct DenseMapInfo<const SwitchSuccWrapper *> {
  ...
  static unsigned getHashValue(const SwitchSuccWrapper *SSW) {
    ...
    // successor, we hash as the BB and the incoming Values of its
    // successor PHIs. Initially, we tried to just use the successor BB as
    // the hash, but including the incoming PHI values leads to better
    // performance. We also tried to build a map from BB ->
    // Succs.IncomingValues ahead of time and passing it in
    // SwitchSuccWrapper, but this slowed down the average compile time
    // without having any impact on the worst case compile time.
...
bool SimplifyCFGOpt::simplifyDuplicateSwitchArms(SwitchInst *SI,
                                                 DomTreeUpdater *DTU) {
  // Build Cases. Skip BBs that are not candidates for simplification.
  // Mark PHINodes which need to be processed into PhiPredIVs. We decide
  // to process an entire PHI at once after the loop, opposed to calling
  ...
  for (unsigned I = 0; I < SI->getNumSuccessors(); ++I) {
    ...
    // FIXME: Support more than just a single BranchInst. One way we could
    // do this is by taking a hashing approach of all insts in BB.
    if (BB->size() != 1)
      continue;

    // FIXME: This case needs some extra care because the terminators
    // other than SI need to be updated. For now, consider only backedges
    // to the SI.
    if (BB->hasNPredecessorsOrMore(4) ||
        BB->getUniquePredecessor() != SI->getParent())
      continue;

    // FIXME: Relax that the terminator is a BranchInst by checking for
    // equality on other kinds of terminators. We decide to only support
    // unconditional branches for now for compile time reasons.
  ...
  // another SwitchSuccWrapper isEqual, then the equivalent
  // SwitchSuccWrapper which is not in the set should be replaced with the
  // one in the set. If the SwitchSuccWrapper is not in the set, then it
  // should be added to the set so other SwitchSuccWrappers can check
  // against it in the same manner. We use SwitchSuccWrapper instead of
  // just BasicBlock because we'd like to pass around information to
  // isEquality, getHashValue, and when doing the replacement with better
  // performance.
  ...
  for (auto &SSW : Cases) {
    ...
    if (!Inserted) {
      // We know that SI's parent BB no longer dominates the old case
      // successor since we are making it dead.
...
bool SimplifyCFGOpt::simplifySwitch(SwitchInst *SI, IRBuilder<> &Builder) {
  ...
  if (isValueEqualityComparison(SI)) {
    // If we only have one predecessor, and if it is a branch on this
    // value, see if that predecessor totally determines the outcome of
    // this switch.
  ...
  // difficult-to-analyze code and makes pruning branches much harder.
  // This is a problem if the switch expression itself can still be
  // restricted as a result of inlining or CVP. Therefore, only apply this
  // transformation during late stages of the optimisation pipeline.
...
/// Given an block with only a single landing pad and a unconditional
/// branch try to find another basic block which this one can be merged
/// with.  This handles cases where we have multiple invokes with unique
/// landing pads, but a shared handler.
///
/// We specifically choose to not worry about merging non-empty blocks
/// here.  That is a PRE/scheduling problem and is best solved elsewhere.
/// In practice, the optimizer produces empty landing pad blocks quite
/// frequently when dealing with exception dense code.  (see: instcombine,
/// gvn, if-else sinking in this file)
///
/// This is primarily a code size optimization.  We need to avoid
/// performing any transform which might inhibit optimization (such as our
/// ability to specialize a particular handler via tail commoning).  We do
/// this by not merging any blocks which require us to introduce a phi.
/// Since the same values are flowing through both blocks, we don't lose
/// any ability to specialize.  If anything, we make such specialization
/// more likely.
///
/// TODO - This transformation could remove entries from a phi in the
/// target block when the inputs in the phi are the same for the two
/// blocks being merged.  In some cases, this could result in removal of
/// the PHI entirely.
static bool tryToMergeLandingPad(LandingPadInst *LPad, BranchInst *BI,
                                 BasicBlock *BB, DomTreeUpdater *DTU) {
  auto Succ = BB->getUniqueSuccessor();
  assert(Succ);
  // If there's a phi in the successor block, we'd likely have to
  // introduce a phi into the merged landing pad block.
  ...
  for (BasicBlock *OtherPred : predecessors(Succ)) {
    ...
    // We've found an identical block.  Update our predecessors to take
    // that path instead and make ourselves dead.
...
bool SimplifyCFGOpt::simplifyUncondBranch(BranchInst *BI,
  ...
  // block. If LoopHeader is provided, check if the block or its successor
  // is a loop header. (This is for early invocations before loop simplify
  // and vectorization to keep canonical loop forms for nested loops.
  // These blocks can be eliminated when the pass is invoked later in the
  // back-end.) Note that if BB has only one predecessor then we do not
  ...
  // predecessor branches to us and our successor, fold the comparison
  // into the predecessor and use logical operations to update the
  // incoming value for PHI nodes in common successor.
  if (Options.SpeculateBlocks &&
...
bool SimplifyCFGOpt::simplifyCondBranch(BranchInst *BI, IRBuilder<> &Builder) {
  assert(!isa<ConstantInt>(BI->getCondition()) &&
         BI->getSuccessor(0) != BI->getSuccessor(1) &&
         "Tautological conditional branch should have been eliminated "
         "already.");
  ...
  if (isValueEqualityComparison(BI)) {
    ...
    // exists. Ignore dbg and pseudo intrinsics.
  ...
  // If this basic block has dominating predecessor blocks and the
  // dominating blocks' conditions imply BI's condition, we know the
  // direction of BI.
  ...
  // If this basic block is ONLY a compare and a branch, and if a
  // predecessor branches to us and one of our successors, fold the
  // comparison into the predecessor and use logical operations to pick
  ...
  // we can hoist it up to the branching block.
  if (BI->getSuccessor(0)->getSinglePredecessor()) {
    if (BI->getSuccessor(1)->getSinglePredecessor()) {
      ...
      // conditionally execute Successor #0 if it branches to Successor
      // #1.
...
static bool passingValueIsAlwaysUndefined(Value *V, Instruction *I,
  ...
  if (C->isNullValue() || isa<UndefValue>(C)) {
    // Only look at the first use we can handle, avoid hurting compile
    // time with long uselists
    auto FindUse = llvm::find_if(I->uses(), [](auto &U) {
      ...
      case Instruction::URem:
        ...
        // implemented to avoid code complexity as it is unclear how
        // useful such logic is.
    ...
    // Bail out if User is not in the same BB as I or User == I or User
    // comes before I in the block. The latter two can be the case if User
    // is a PHI node.
    if (User->getParent() != I->getParent() || User == I ||
        User->comesBefore(I))
      return false;

    // Now make sure that there are no instructions in between that can
    // alter control flow (eg. calls)
    auto InstrRange =
    ...
    if (GetElementPtrInst *GEP = dyn_cast<GetElementPtrInst>(User))
      if (GEP->getPointerOperand() == I) {
        ...
        // Bail out on vector GEPs, as they are not handled by other
        // checks.
        if (GEP->getType()->isVectorTy())
          return false;
        // The current base address is null, there are four cases to
        // consider: getelementptr (TY, null, 0)                 -> null
        // getelementptr (TY, null, not zero)          -> may be modified
        // getelementptr inbounds (TY, null, 0)        -> null
        // getelementptr inbounds (TY, null, not zero) -> poison iff null
        // is undefined?
...
/// If BB has an incoming value that will always trigger undefined
/// behavior (eg. null pointer dereference), remove the branch leading
/// here.
    ...
    for (unsigned i = 0, e = PHI.getNumIncomingValues(); i != e; ++i)
      if (passingValueIsAlwaysUndefined(PHI.getIncomingValue(i), &PHI)) {
        ...
        if (BranchInst *BI = dyn_cast<BranchInst>(T)) {
          ...
          // Turn unconditional branches into unreachables and remove the
          // dead destination from conditional branches.
          if (BI->isUnconditional())
            Builder.CreateUnreachable();
          else {
            // Preserve guarding condition in assume, because it might not
            // be inferrable from any dominating condition.
            Value *Cond = BI->getCondition();
            CallInst *Assumption;
            if (BI->getSuccessor(0) == BB)
              Assumption = Builder.CreateAssumption(Builder.CreateNot(Cond));
...
bool SimplifyCFGOpt::simplifyOnce(BasicBlock *BB) {
  ...
  if (SinkCommon && Options.SinkCommonInsts)
    if (sinkCommonCodeFromPredecessors(BB, DTU) ||
      ...
      // foldTwoEntryPHINode() potentially converts them into
      // select's, after which we'd need a whole EarlyCSE pass run
      // to cleanup them.
      return true;
    }
  ...
  if (Options.SpeculateBlocks &&
    ...
    // If there is a trivial two-entry PHI node in this basic block,
    // and we can eliminate it, do so now.
...
bool SimplifyCFGOpt::run(BasicBlock *BB) {
  ...
  do {
    ...
    // Perform one round of simplifcation. Resimplify flag will be
    // set if another iteration is requested.
