...
static cl::opt<bool> GVNEnablePRE("gvn-local-enable-pre", cl::init(true),
                 cl::Hidden);
static cl::opt<bool> GVNEnableLoadPRE("gvn-local-enable-load-pre",
                    cl::init(true));
static cl::opt<bool> GVNEnableLoadInLoopPRE(
  "gvn-local-enable-load-in-loop-pre", cl::init(true));
static cl::opt<bool> GVNEnableSplitBackedgeInLoadPRE(
  "gvn-local-enable-split-backedge-in-load-pre", cl::init(false));
static cl::opt<bool> GVNEnableMemDep("gvn-local-enable-memdep",
                   cl::init(true));
static cl::opt<bool> GVNEnableMemorySSA("gvn-local-enable-memoryssa",
                    cl::init(false));

static cl::opt<uint32_t> MaxNumDeps(
  "gvn-local-max-num-deps", cl::Hidden, cl::init(100),
...
static cl::opt<uint32_t> MaxBBSpeculations(
  "gvn-local-max-block-speculations", cl::Hidden, cl::init(600),
...
static cl::opt<uint32_t> MaxNumVisitedInsts(
  "gvn-local-max-num-visited-insts", cl::Hidden, cl::init(100),
...
static cl::opt<uint32_t> MaxNumInsnsPerBlock(
  "gvn-local-max-num-insns", cl::Hidden, cl::init(100),
...
PreservedAnalyses GVNPass::run(Function &F, FunctionAnalysisManager &AM) {
  ...
  errs() << "Running GVN on function: " << F.getName() << "\n";
...
void GVNPass::eliminatePartiallyRedundantLoad(
  ...
  for (const auto &AvailableLoad : AvailableLoads) {
    ...
    if (CriticalEdgePredAndLoad) {
      ...
      if (It != CriticalEdgePredAndLoad->end()) {
        ...
        OldLoad->replaceAllUsesWith(NewLoad); GVNReplacements.push_back({getValueNameLocal(OldLoad), getValueNameLocal(NewLoad)});
  ...
  Load->replaceAllUsesWith(V); GVNReplacements.push_back({getValueNameLocal(Load), getValueNameLocal(V)});
  if (isa<PHINode>(V))
    V->takeName(Load);
...
static void patchAndReplaceAllUsesWith(Instruction *I, Value *Repl) {
  ...
  I->replaceAllUsesWith(Repl); GVNReplacements.push_back({getValueNameLocal(I), getValueNameLocal(Repl)});
}
...
bool GVNPass::processLoad(LoadInst *L) {
  ...
  errs() << "available value: " << *AvailableValue << "\n";
  // MaterializeAdjustedValue is responsible for combining metadata.
  ICF->removeUsersOf(L);
  L->replaceAllUsesWith(AvailableValue); GVNReplacements.push_back({getValueNameLocal(L), getValueNameLocal(AvailableValue)});
  if (MSSAU)
...
bool GVNPass::processInstruction(Instruction *I) {
  ...
  if (Value *V = simplifyInstruction(I, {DL, TLI, DT, AC})) {
    ...
    if (!I->use_empty()) {
      ...
      ICF->removeUsersOf(I);
      I->replaceAllUsesWith(V); GVNReplacements.push_back({getValueNameLocal(I), getValueNameLocal(V)});
      Changed = true;
    }
    if (isInstructionTriviallyDead(I, TLI)) {
      errs() << "removing trivially dead instruction: " << *I << "\n";
  ...
  errs() << "value number for instruction: " << *I << " is " << Num << "\n";
  errs() << "next unused value number is: " << NextNum << "\n";
  ...
  errs() << "replacing instruction: " << *I << " with: " << *Repl << "\n";
  // Remove it!
...
bool GVNPass::performScalarPRE(Instruction *CurInst) {
  ...
  CurInst->replaceAllUsesWith(Phi); GVNReplacements.push_back({getValueNameLocal(CurInst), getValueNameLocal(Phi)});
...
INITIALIZE_PASS_BEGIN(GVNLegacyPass, "gvn-local",
                      "Global Value Numbering (local)", false, false)
...
INITIALIZE_PASS_END(GVNLegacyPass, "gvn-local",
                    "Global Value Numbering (local)", false, false)
